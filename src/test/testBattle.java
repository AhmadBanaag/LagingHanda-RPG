...
public class testBattle {
	public static void main(String[] args) {
		...
		int enemyInput = dice.nextInt(1, 6);
		
		//HERO CONDITIONS
		switch (heroInput) {
		case 1:
			int userAttack = dice.nextInt(0, 11) + dice.nextInt(0, 21);
			firstEnemyHealth = (firstEnemyHealth - userAttack);
		...
		case 2:
			...
			System.out.println("You've been healed with " + heal + " HP!");
			
			if (heal >= 1 && heal <= 7) {
				System.out.println("\nYou heal just about right");
			} else if (heal >= 8 && heal <= 12) {
				System.out.println("\nYou're luck is up, eh?");
			} else if (heal >= 13 && heal <= 15) {
				System.out.println("\nNICE ONE DOCTOR!");
			} else {
				System.out.println("\nAhhhh, don't know about that...");
			}
			break;
		case 3:
			System.out.println("You attempt to flee...");
			
			int escape = dice.nextInt(1, 6);
			int run = dice.nextInt(1, 6);
		...
		if (enemyAttack >= 1 && enemyAttack <= 10) {
			...
			System.out.println("The " + firstEnemy + " was healed with " + heal + " HP!" );
			break;
		}
		
		System.out.println("Enemy HP: " + firstEnemyHealth + "\nYOUR HP: " + heroHealth + "\n\n=================================\n");
